...
| `src/AnalysisProgress.tsx`              | 경과 시간, 실제 실행 단계, 마지막 상태 확인    |
...
리밸런싱은 정확한 현재 평가액과 환율을 사용하며 화면에서 반올림한 비중을 재사용하지 않는다. 최대 종목 비중, 현금 하한, 중복 목표를 검증한다. 신규 자금 방식의 매수 요구가 예산을 넘으면 비례 축소한다. 최종 금액은 센트 단위이며 세금·수수료·가격 변화를 제외한다.
