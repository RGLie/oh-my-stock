...
`dailyBrief`에는 시장 상태, 출처·기준 시각이 있는 지수와 변동 이유(현재·전일), 메인 뉴스, 주요 기업(전일·현재 주가 변동 이유), 섹터 동향, 경제·실적·시장 일정, 우선 확인 항목이 포함된다. 오늘 일정과 나머지 가까운 일정을 분리해서 표시한다. 브리핑 날짜 불일치, 출처 없는 지수·일정·뉴스·기업·섹터, 존재하지 않는 인용 ID는 검증 실패로 처리한다. 다른 분석에서는 `dailyBrief=null`을 사용하며 기존 저장 결과의 누락 필드는 허용한다. 스키마 파일명은 내용 해시를 사용해 이전 CLI 스키마 캐시의 영향을 막는다.
...
동기화·시세 갱신·보유 및 현금 변경, 서버 실행 중 15분 간격으로 스냅샷을 저장한다. 현재 화면은 최근 300개를 `Store.recent`로 조회하며 전체 기록은 DB·백업에 남는다. 스냅샷은 합계(`usd`/`krw`) 외에 `stockUsd`, `cashUsd`(USD 환산), `fxRate`, 종목별 `positions`(수량·USD 가격), 직전 스냅샷 대비 `flows`와 합계 `flowUsd`를 저장한다. 흐름은 수량 변화 × 현재 가격과 현금 변화로 계산하고, 가격이 새로 생기거나 사라진 종목은 측정 가능한 가치 전체를 흐름으로 잡는다. `positions`는 `/api/state`에 내려보내지 않는다. 화면의 세 가지 보기(총자산·주식만·성과 지수)는 `shared/performance.ts`가 같은 스냅샷 배열에서 계산한다. 성과 지수는 흐름 데이터가 있는 첫 스냅샷에서 100으로 시작해 `(V_t − 누적 flow) / V_{t−1}`을 이어 붙이며, 불완전한 스냅샷의 흐름은 다음 완전한 스냅샷에 합산한다. 과거 스냅샷 값은 수정하지 않는다.
