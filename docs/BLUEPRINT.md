...
제안의 품질 기준은 확신의 강도가 아니라 검증 가능성이다. 유지가 아닌 모든 제안에는 비중 수치 또는 범위(`proposedWeight`)가 있어야 하며, 서버는 이를 위반한 응답을 거부한다. 각 제안은 근거의 질에 따른 3단계 확신도(`conviction`: high/medium/low, 숫자 확률 금지)와 제안을 철회할 조건(`invalidation`: 지표·비교 방향·임계값·확인 시점)을 포함한다. 수치 기준이 없는 재검토 조건은 실패로 처리하지 않되 화면에 경고로 표시한다. "비중을 줄여야 한다", "실적을 확인하면"처럼 크기나 기준이 없는 문장은 공통 지침에서 금지한다.
